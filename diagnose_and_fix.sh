#!/bin/bash
# STAAX — Blank page diagnosis + fix
# cd ~/STAXX/staax && bash diagnose_and_fix.sh

echo "🔍 Diagnosing blank page..."
echo ""

# ── 1. Check we're in the right place ────────────────────────────────────────
if [ ! -f "package.json" ] && [ ! -f "frontend/package.json" ]; then
  echo "❌ Not in project root. Run: cd ~/STAXX/staax"
  exit 1
fi
...
# ── 3. Check for broken imports in all page files ────────────────────────────
echo "Scanning for broken imports..."
BROKEN=0

for f in $SRC/pages/*.tsx $SRC/App.tsx $SRC/components/**/*.tsx; do
  [ -f "$f" ] || continue
  if grep -q "AlgoContext\|useAlgos" "$f"; then
    echo "  ❌ BROKEN IMPORT in $f:"
    grep -n "AlgoContext\|useAlgos" "$f"
    BROKEN=1
  fi
done

# Check for imports of pages that don't exist
while IFS= read -r line; do
  page=$(echo "$line" | grep -oP "from '@/pages/\K[^']+")
  [ -z "$page" ] && continue
  if [ ! -f "$SRC/pages/$page.tsx" ]; then
    echo "  ❌ $SRC/App.tsx imports missing file: $page.tsx"
    BROKEN=1
  fi
done < "$SRC/App.tsx"

[ $BROKEN -eq 0 ] && echo "  ✅ No broken imports found"
echo ""

# ── 4. Fix all broken imports ─────────────────────────────────────────────────
echo "Applying fixes..."

# Fix any AlgoContext/useAlgos references
for f in $SRC/pages/*.tsx; do
  [ -f "$f" ] || continue
  if grep -q "AlgoContext\|useAlgos" "$f"; then
    # Remove import line
    sed -i.bak "s/^import.*AlgoContext.*$//g" "$f"
    sed -i.bak "s/^import.*useAlgos.*$//g" "$f"
    # Replace usage
    sed -i.bak "s/const {addAlgo} = useAlgos()/const addAlgo = (_: any) => {}/g" "$f"
    sed -i.bak "s/const {addAlgo}=useAlgos()/const addAlgo=(_:any)=>{}/g" "$f"
    sed -i.bak "s/const { removeAlgo } = useAlgos()/\/\/ removeAlgo stubbed/g" "$f"
    rm -f "${f}.bak"
    echo "  ✅ Fixed: $f"
  fi
done

# ── 5. Rewrite GridPage with zero external dependencies ───────────────────────
echo "Writing clean GridPage.tsx..."
cat > $SRC/pages/GridPage.tsx << 'GRID'
    ...
    <div style={{overflowX:'auto'}}>
      <table style={{width:'100%',borderCollapse:'collapse'}}>
        <colgroup>
          ...
          {days.map(d=><col key={d} style={{minWidth:'115px'}}/>)}
        </colgroup>
        <thead>
          <tr>
            ...
            return <tr key={algo.id}>
              ...
              {days.map(day=>{
                ...
                return <td key={day} onDragOver={e=>e.preventDefault()} onDrop={()=>onDrop(algo.id,day)}
                  ...
                  {cell&&s?<div style={{background:'var(--bg-secondary)',borderLeft:`3px solid ${s.col}`,borderRadius:'5px',padding:'6px 8px',position:'relative'}}>
                      ...
                      <div style={{display:'flex',alignItems:'center',gap:'4px'}}>
                        ...
                        {ed?.id===algo.id&&ed?.day===day
                          ?<input autoFocus type="number" min={1} value={ev}
                            ...
                            style={{width:'30px',background:'var(--bg-primary)',border:'1px solid var(--accent-blue)',borderRadius:'2px',color:'var(--text)',fontSize:'10px',padding:'0 3px',fontFamily:'inherit'}}/>
...
GRID
echo "✅ GridPage.tsx written"

# ── 6. Rewrite App.tsx to only import pages that exist ───────────────────────
echo "Writing App.tsx..."

# Build import list based on what files exist
IMPORTS="import { BrowserRouter, Routes, Route, Navigate } from 'react-router-dom'
...
ROUTES="          <Route path=\"grid\"       element={<GridPage />} />
...
import DashboardPage from '@/pages/DashboardPage'"
  ROUTES="          <Route path=\"dashboard\"  element={<DashboardPage />} />
$ROUTES"
  DEFAULT_ROUTE="/dashboard"
fi

if [ -f "$SRC/pages/IndicatorsPage.tsx" ]; then
  IMPORTS="$IMPORTS
...
APPEOF
echo "✅ App.tsx written (default route: $DEFAULT_ROUTE)"
...
echo ""
echo "If still blank, open DevTools (F12 → Console tab)"
echo "and paste any red errors here."
echo "═══════════════════════════════════════"
